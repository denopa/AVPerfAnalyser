...
app.secret_key = (
    b"\xef<\xf0\xd6KE\x82\x11\xa1\x99-\x9b\xf0\x1a(\xe6-\xff\x7f[\xff\xc0k\xe9"
)
try:
...
class uploadFlight(Resource):
    def allowedFiles(self, filename):
        ...
        if (
            re.search(r"([^.]*)$", filename).group().upper()
    ...
    def post(self):
        ...
        approachType = request.form["approachType"]
        if csv.filename == "":
            print("No filename")
            return "No filename"
        if self.allowedFiles(csv.filename):
            ...
            flightAnalysis = analyseFlight(
                takeoffWeight, takeoffMethod, approachType, csvFileName
            )
            ...
            return make_response(
                render_template(
                    "flightResults.html",
                    meta=flightAnalysis["meta"],
                    tables=[
                        table.to_html(classes="mystyle")
                        for table in flightAnalysis["tables"]
                    ],
                    titles=[
                        "",
                        "Summary",

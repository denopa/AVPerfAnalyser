...
def getPerf(book, x, units):
    ...
    for (
        ...
        x
    ):  # make sure the value to look in the book for is inded in the book, replace by the limit otherwise
        x[i] = min(
            book.index.get_level_values(i).max(),
            max(ix, book.index.get_level_values(i).min()),
        )
        i = i + 1
    ...
    if flightPart == "climb":
        ...
        with open("models/" + model + "/" + flightPart + ".csv") as dataFile:
            book = pd.read_csv(
                dataFile,
                index_col=[
                    "cruisePowerVariable1",
                    "cruisePowerVariable2",
                    "tempVISA",
                    "pressAlt",
                ],
            )
        ...
        with open("models/" + model + "/" + flightPart + ".csv") as dataFile:
            book = pd.read_csv(
                dataFile, index_col=["tempVISA", "pressAlt", "weight", "headwind"]
            )
    else:
        configuration = kwargs.get("configuration", "standard")
        if configuration == "standard":
            with open(
                "models/" + model + "/" + flightPart + "Standard.csv"
            ) as dataFile:
                book = pd.read_csv(
                    dataFile, index_col=["tempVISA", "pressAlt", "weight", "headwind"]
                )
...
def engineMetrics(flight, flightTable, modelConfig, flightPart):
    flightTable.loc[flightPart + " Average Fuel Flow"] = [
        round(flight["E1 FFlow"].mean(), 1),
        "-",
        "-",
        "USG",
    ]
    if "E1 CHT1" in flight.columns:
        maxCHT = (
            flight[["E1 CHT1", "E1 CHT2", "E1 CHT3", "E1 CHT4", "E1 CHT5", "E1 CHT6"]]
            .max()
            .max()
        )
        highestAverageCHT = (
        ...
        maxEGTSpread = (
            flight[["E1 EGT1", "E1 EGT2", "E1 EGT3", "E1 EGT4", "E1 EGT5", "E1 EGT6"]]
            .apply(maxSpread)
            .max()
        )
        flightTable.loc[flightPart + " Max CHT"] = [
            round(maxCHT),
            round(float(modelConfig.loc["maxCHT", "Value"])),
            round(100 * (maxCHT / float(modelConfig.loc["maxCHT", "Value"]) - 1)),
            "degrees F",
        ]
        flightTable.loc[flightPart + " Highest Average CHT"] = [
            round(highestAverageCHT),
            "-",
            "-",
            "degrees F",
        ]
        flightTable.loc[flightPart + " Hottest Average Cylinder"] = [
            highestAverageCylinder,
            "-",
            "-",
            "-",
        ]
        flightTable.loc[flightPart + " Max CHT Spread"] = [
            round(maxCHTSpread),
            "-",
            "-",
            "degrees F",
        ]
        flightTable.loc[flightPart + " Max EGT Spread"] = [
            round(maxEGTSpread),
            "-",
            "-",
            "degrees F",
    ...
    if "E1 OilT" in flight.columns:
        flightTable.loc[flightPart + " Average Oil Temp"] = [
            round(flight["E1 OilT"].mean()),
            "-",
            "-",
            "degrees F",
        ]
    if "E1 OilP" in flight.columns:
        flightTable.loc[flightPart + " Average Oil Press"] = [
            round(flight["E1 OilP"].mean()),
            "-",
            "-",
            "PSI",
        ]
    if "E1 FPres" in flight.columns:
        flightTable.loc[flightPart + " Average Fuel Press"] = [
            round(flight["E1 FPres"].mean()),
            "-",
            "-",
            "PSI",
        ]
    if "E1 CDT" in flight.columns:
        averageICEfficiency = (
            100
            * (
                (flight["E1 CDT"] - flight["E1 IAT"])
                / (flight["E1 CDT"] - flight["OAT"].apply(c2f))
            ).mean()
        )
        flightTable.loc[flightPart + " Intercooler Efficiency"] = [
            round(averageICEfficiency),
            "-",
            "-",
            "%",
        ]

...
def takeoffPerformance(flight, model, modelConfig, takeoffMethod, takeoffWeight):
    ...
    (
        ...
        windDirection,
        track,
    ) = calcGroundRoll(flight, modelConfig)
    ...
    bookTakeoffRoll = getPerf(
        takeoffRollBook,
        [isaDiff(temp, pressAlt), pressAlt, takeoffWeight, headwind],
        runwayUnits,
    )
    bookDistanceOver50 = getPerf(
        distanceOver50Book,
        [isaDiff(temp, pressAlt), pressAlt, takeoffWeight, headwind],
        runwayUnits,
    )
    ...
    takeoffTable.loc["Takeoff IAS"] = [
        int(takeoffAIS),
        int(bookTakeoffIAS),
        round(100 * (takeoffAIS / bookTakeoffIAS - 1)),
        "knots",
    ]
    takeoffTable.loc["Takeoff Roll"] = [
        int(takeoffRoll),
        int(bookTakeoffRoll),
        round(100 * (takeoffRoll / bookTakeoffRoll - 1)),
        runwayUnits,
    ]
    takeoffTable.loc["Takeoff Dist. over 50 feet"] = [
        int(fiftyFeetDistance),
        int(bookDistanceOver50),
        round(100 * (fiftyFeetDistance / bookDistanceOver50 - 1)),
        runwayUnits,
    ]
    takeoffTable.loc["Takeoff AIS over Barrier"] = [
        int(barrierIAS),
        int(bookBarrierIAS),
        round(100 * (barrierIAS / bookBarrierIAS - 1)),
        "knots",
    ]
    takeoffTable.loc["Takeoff Headwind"] = [round(headwind), "-", "-", "knots"]
    takeoffTable.loc["Takeoff Crosswind"] = [round(crosswind), "-", "-", "knots"]
    takeoffTable.loc["Takeoff Temp vs ISA"] = [
        round(isaDiff(temp, pressAlt)),
        "-",
        "-",
        "degrees C",
    ]

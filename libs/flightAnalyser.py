...
def cleanUp(flight):  # put everything in the right format
    ...
    numericals = flight.columns.drop(
        ["GPSfix", "HSIS", "Lcl Date", "Lcl Time", "UTCOfst", "AtvWpt"]
    )
    ...
    flight["hour"] = pd.to_numeric(
        flight["Lcl Time"].apply(lambda x: x[:3]), errors="coerce"
    ) - pd.to_numeric(flight["UTCOfst"].apply(lambda x: x[:5]), errors="coerce")
    flight["minute"] = flight["Lcl Time"].apply(lambda x: x[4:6])
    flight["second"] = flight["Lcl Time"].apply(lambda x: x[7:9])
    flight["datetime"] = pd.to_datetime(
        flight[["year", "month", "day", "hour", "minute", "second"]], errors="coerce"
    )
    flight.drop(
...
def analyseFlight(takeoffWeight, takeoffMethod, approachType, csvFileName):
    ...
    fuelEnd = (
        flight.loc[flight.index.max(), "FQtyL"]
        + flight.loc[flight.index.max(), "FQtyR"]
    )
    ...
    if "E1 TIT1" in flight.columns:
        ...
        maxTIT = None
    flightDuration = round(
        (flight["datetime"].max() - flight["datetime"].min()).seconds / 3600, 1
    )
    continousData = (
        0.97 * len(flight) / 3600 < flightDuration < 1.03 * len(flight) / 3600
    )
    meta = {
        ...
        "continuousData": continousData,
    }
    flightSummary = pd.DataFrame.from_dict(
        {
            "Flight Fuel Start": int(fuelStart),
            "Flight Fuel End": int(fuelEnd),
            "Flight Max Altitude": int(maxAltitude),
            "Flight Max Positive Load": round(maxG, 2),
            "Flight Max Negative Load": round(minG, 2),
            "Flight Max IAS": int(maxIAS),
            "Flight Max TAS": int(maxTAS),
            "Flight Max GS": int(maxGS),
            "Flight Max CHT": int(maxCHT),
            "Flight Max TIT": "-",
        },
        orient="index",
        columns=["Actual"],
    ...
    try:
        takeoffAnalysis, takeoffStability = takeoffPerformance(
            flight, model, modelConfig, takeoffMethod, takeoffWeight
        )
    except:
    ...
    try:
        approachAnalysis, approachStability = approachPerformance(
            flight, model, modelConfig, approachType, takeoffWeight
        )
    except:

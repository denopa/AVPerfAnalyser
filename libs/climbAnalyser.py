...
def climbPerformance(flight, model, modelConfig):
    ...
    climbTable.loc['Climb Time'] = [round(climbTime), round(bookClimbPerf[0]), round(100*(climbTime/bookClimbPerf[0]-1)), 'minutes']
    climbTable.loc['Climb Fuel Used'] = [round(totalClimbFuel,1),round(bookClimbPerf[1],1), round(100*(totalClimbFuel/bookClimbPerf[1]-1)), "USG"]
    climbTable.loc['Climb Fuel Used per 10k feet'] = [round(totalClimbFuel/climbAlt*10000,1),round(bookClimbPerf[1]/climbAlt*10000,1), round(100*(totalClimbFuel/bookClimbPerf[1]-1)), "USG"]
    climbTable.loc['Climb Average Vertical Speed'] = [round(climbAlt/climbTime),round(climbAlt/bookClimbPerf[0]),round(100*(1-climbTime/bookClimbPerf[0])),'fpm']
    climbTable.loc['Climb Average IAS'] = [round(climb['IAS'].mean()),'-','-','knots']
    climbTable.loc['Climb Average Power'] = [round(climbPower,1),str(climbBook.index.get_level_values(0).min())+'-'+str(climbBook.index.get_level_values(0).max()),'-',climbPowerIndicator]
    if 'climbMaxTIT' in modelConfig.index:
        ...
        climbTable.loc['Climb Max TIT'] = [round(maxClimbTIT), modelConfig.loc['climbMaxTIT','Value'], round(100*(maxClimbTIT/float(modelConfig.loc['climbMaxTIT','Value'])-1)),'degrees F']
    climbTable = engineMetrics(climb, climbTable, modelConfig, 'Climb')
    climbTable.loc['Climb Average temp vs ISA'] = [round(climbISA,1),'-','-','degrees C']
    return(climbTable)
